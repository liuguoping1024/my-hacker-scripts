...
async def websocket_get_network_settings(
    ...
    original_backup = async_get_active_network_settings(hass)
    _LOGGER.info("zha/network/settings backup: %s", original_backup)

    # Create a deep copy of the original backup
    backup_local = copy.deepcopy(original_backup)

    try:
        # Attempt to convert network_key to a dictionary
        backup_local.network_info.network_key.as_dict()
    except (TypeError, AttributeError):
        backup_local.network_info.network_key = backup_local.network_info.tc_link_key

    try:
        # Use the local backup to send settings
        settings = backup_local.as_dict()
    except HomeAssistantError as err:
        _LOGGER.error("Failed to serialize local backup: %s", err)
        connection.send_error(msg[ID], "serialization_error", "Failed to process backup settings")
        return
